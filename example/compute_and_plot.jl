...
function compute_and_plot(df_cases::DataFrame, case_name::String, k_gen::Int, ylabel_R::String, ylabel_N::String, databasis::String)
    ...
    plot(R_4_days.days, R_4_days.R, label=label_4_days, ylabel=ylabel_R, title="R based on $(databasis). R of the projected 7 estimator is $(round(last_number; digits=2)). (Last updated: $(today()).)", titlefontsize=7, margin=7mm)
    ...
    gr()
    plot(N_4_days.days, N_4_days.cases, label=label_4_days, ylabel=ylabel_N, title="Number of cases based on $(databasis). (Last updated: $(today()).) ", titlefontsize=7, margin=7mm)
